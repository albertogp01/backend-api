...
async function generatePDF(htmlContent, clientName = "Cliente", tempDir = "", requestId = "") {
  return new Promise(async (resolve, reject) => {
    ...
    try {
      ...
      // --- Preparar logo ---
      // Asegúrate de que la ruta relativa sea correcta desde donde se ejecuta pdfService.js
      ...
      if (fs.existsSync(logoPath)) {
        ...
        // Considera si quieres continuar sin logo o rechazar la promesa aquí
      }
      ...
      // Llamar a createCoverPage para obtener todos los componentes necesarios
      const { fullCoverPageHtml, styles, script, scores, volumeData } = createCoverPage(htmlContent, clientName, logoBase64);

      // Combinar los componentes de la portada en un solo string HTML
      // Importante: El script del gráfico ya está incluido aquí por getRadarChartScript
      ...
      // console.log("Scores para portada:", scores); // Descomenta para depurar scores si es necesario

      // --- Ensamblar el HTML COMPLETO para TODO el PDF (Portada + Rutina) ---
      // Se generarán como páginas separadas en Puppeteer
      const routinePageHtml = `
        ...
        <head>
            ...
            <style>
                ...
                /* ... (Tu CSS para la rutina va aquí, sin cambios) ... */
                 :root {
      ...
      browser = await puppeteer.launch({
        headless: true, // 'true' es más compatible que 'new'
        args: [
          "--no-sandbox",
          "--disable-setuid-sandbox",
          "--disable-dev-shm-usage", // Necesario en entornos limitados
          "--disable-gpu", // A menudo innecesario para headless, puede ayudar
          // "--js-flags=--max-old-space-size=2048", // Descomentar si hay errores de memoria
          "--disable-web-security" // Puede ayudar con CORS si cargas recursos externos
        ],
        // executablePath: '/usr/bin/google-chrome-stable' // Descomentar y ajustar si Puppeteer no encuentra Chrome/Chromium
      });
      ...
      // Configurar viewport (opcional pero recomendado para consistencia)
      ...
      // Usar waitUntil: 'networkidle0' para esperar que Chart.js (cargado desde CDN) se cargue
      await page.setContent(coverPageCompleteHtml, { waitUntil: 'networkidle0', timeout: 60000 }); // Aumentar timeout
      console.log("Contenido de la portada establecido. Esperando renderizado...");

      // Opcional: Añadir una pequeña espera adicional explícita si 'networkidle0' no es suficiente
      // await page.waitForTimeout(2000); // Espera 2 segundos adicionales

      // Verificar si el canvas del gráfico se ha renderizado (opcional, para depuración)
      const chartRendered = await page.evaluate(() => {
          const canvas = document.getElementById('radarChart');
          // Chart.js 3.x podría no añadir un canvas hijo, comprobamos si el propio canvas tiene contenido
          return canvas && canvas.toDataURL().length > 1000; // Comprobar si el canvas tiene datos
      });
      console.log(`¿Gráfico radar renderizado?: ${chartRendered}`);
